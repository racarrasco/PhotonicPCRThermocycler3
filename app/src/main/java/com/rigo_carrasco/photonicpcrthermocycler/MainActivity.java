...
public class MainActivity extends AppCompatActivity {
    ...
    ServiceConnection btConnection;
    boolean isBound;
    ...
    private MyHandler mHandler; //handler for the service
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Bind to Service
        btConnection = new ServiceConnection() {
            @Override
            public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
                isBound = true;
                myService = ((BluetoothService.LocalBinder) iBinder).getService();
    ...
    public void pushcmd(String command) { //method that writes commands to arduino
        myService.writeData(command.getBytes());
    }
    ...
    public void onClickRun(View view) {
        final List<Map<String, Object>> cmd = encodecmd(parametersActivity);//encode commands for the arduino

        Body = "\nPCR started on: " + DateFormat.getDateTimeInstance().format(new Date());
        tvSet(current_cycle,Integer.toString(1));
        controlButton.setEnabled(false);
        runButton.setEnabled(false);
        removeDataSet(); //clears the displayed data: basically removes any data on the graph and "restarts the thermocycling"
        current_cycle.setText(" ");
        temperature.setText(" ");
        ...
        Thread longthread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Run(cmd); //send the commands to the arduino
                sendDataButton.setEnabled(false);
                if(internalFile!=null)
                    internalFile.delete();
    ...
    public void onResume() {
        super.onResume();
        startService(BluetoothService.class, btConnection, null);// Start BluetoothService(if it was not started before) and Bind it
    }
    ...
    public void plotData(ArrayList<Float> time, ArrayList<Float> temp) { // plots the time and temperature data
        removeDataSet();
    ...
    private void showCyclingParameters(final ArrayList<Float> temp ) { //plots the parameters on the graph
    ...
    public void highlight(String type,String target) {
        ...
        if(type.equals("O") | type.equals("K")) {
            ...
            if(type.equals("H"))
                sets.get(sets.size()-1).setDrawFilled(false); //Removes the highlighting (the fill)
            for (int i =0;i<sets.size();i++) {
                int j= -1;
                do{
                    j++;
    ...
    /*
    * This handler will be passed to BluetoothService. Data received from serial port is displayed through this handler
    */
       ...
       /* public  float[] sliceArray(float[] arr, int begin,int end, int spacing) {
        ...
        public void update_data(float[] times,float[] temps) { //filters through data received by the arduino
            ...
            if (mActivity.get().overallTime.size() == 0) {
                ...
                dtime_recent = mActivity.get().overallTime.get(mActivity.get().overallTime.size() - 1); // gets the last object in the array
            }
            for (int i = 0; i < times.length; i++) {
                if (times[i] > dtime_recent) { //makes sure that the data received is new data and not repetitive
                    mActivity.get().overallTime.add(times[i]); //add new data
                     ...
                     */
                     if (data.length<100) { //cycling status
                        if (!data[0].equals("0")) { //if pointer isnt 0 prompt arduino to send data
                            ...
                            precooltime = Float.parseFloat(data[4]); //the precooling time
                            if(data.length==7){
                            mActivity.get().tvSet(mActivity.get().temperature, data[5]);
                            mActivity.get().timeElapsed.setText(String.format(Locale.US,"%.2f",Float.parseFloat(data[6])-precooltime));}
                            mActivity.get().overallTime.add(Float.parseFloat(data[6]));
                            mActivity.get().overallTemp.add(Float.parseFloat(data[5]));
                            try {
                            ...
                            mActivity.get().pushcmd("R\n");
                            mActivity.get().clearButton.setEnabled(true); //allow buttons to be pressed
    ...
    public void onClickToParameters(View view) { //Takes user to another screen to set the parameters for thermal cycling
        Intent parametersScreenIntent = new Intent(this, CyclingParameters.class);
        final int result = 1;
